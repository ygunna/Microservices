...
마이크로서비스에 대한 상세 정보 및 아키텍처 관련 지식에 관한 자세한 설명은 <u>[여기](<https://github.com/startupcloudplatform/Sample-App-Tutorial/blob/master/microservice.md>)</u>에서 확인 가능하다.

### 1.1 아키텍처

클라우드 기반 창업플랫폼에서 제공하는 마이크로서비스 스튜디오는 기본적으로 아래와 같은 구조를 하고 있다. 

![MSA_Architecture](https://github.com/startupcloudplatform/Microservices/blob/master/images/MSA_Architecture.png)
...
아래 그림은 마이크로서비스 스튜디오를 실행했을 경우 로그인 화면이다.
...
마이크로서비스아키텍처 메뉴를 통해 마이크로서비스를 생성하려면 PaaSXpert의 배포 메뉴의 마이크로서비스아키텍처를 선택한다. 초기 화면은 마이크로서비스 목록이 조회된다. 조회되는 목록은 현재 선택되어 있는 프로젝트(조직)이 기본으로 보이고 리스트 박스를 통해 프로젝트(조직)을 변경할 수 있다. 
